...
class InteractMove(BaseDataset):
    def __init__(self, data_dir: str) -> None:
        super(InteractMove, self).__init__(data_dir)
        self.num_betas = 10
        self.save_dir = os.path.join('./data/InteractMove/', 'motions')
    ...
    def process(self) -> None:
        print(f'Processing InteractMove dataset in {self.data_dir} ...')

        os.makedirs(self.save_dir, exist_ok=True)
        aligns = natsorted(glob.glob(os.path.join(self.data_dir, 'align_s3', '*', 'anno.pkl')))
        ...
        for align in aligns:
            ...
            for i, p in enumerate(data):
                ...
                if motion_id not in motion_data:
                    with open(os.path.join(self.data_dir, 's3/test', motion_id, 'motion.pkl'), 'rb') as fp:
        ...
        for anno_index in tqdm(range(len(anno_list))):
            ...
            gender, origin_trans, origin_orient, pose_body, pose_left_hand, \
                pose_right_hand, n_comps, joints_traj = motion_data[motion_id]
            ...
            param_seq = np.concatenate([cur_trans, cur_orient, pose_body, pose_left_hand, pose_right_hand], axis=-1)
        ...
        with open('./data/InteractMove/annotations.csv', 'w') as fp:
