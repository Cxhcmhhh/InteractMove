...
def load_interactmove(min_horizon: int, max_horizon: int, **kwargs: Dict) -> Tuple:
    ...
    motion_paths = natsorted(glob.glob('./data/InteractMove/motions_pos/*.npy'))
    anno_file = pd.read_csv('./data/InteractMove/annotations.csv')
    ...
    for motion_path in tqdm(motion_paths, desc='Loading HUMANISE..'):
        ...
        if scene_id not in scene_data:
            scene_pcd = np.load(f'./data/InteractMove/points/{scene_id}.npy') # <x,y,z,r,g,b>
            scene_data[scene_id] = {
                'pcd': scene_pcd.astype(np.float32),
                'mesh_path': f'/network_space/server129/caixh/HUMANISE/scans/{scene_id}/{scene_id}_vh_clean_2.ply'
            }
