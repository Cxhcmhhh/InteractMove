...
def hand_pca_to_axis_angle(hand_pose_48, model_path="body_models/smplx/SMPLX_NEUTRAL.npz"):

    data = np.load(model_path, allow_pickle=True)
    comp_r, comp_l = data["hands_componentsr"], data["hands_componentsl"]  # (45,45)

    # 取前24列
    comp_r = comp_r[:, :24]  # (45,24)
    comp_l = comp_l[:, :24]  # (45,24)

    right_pca = hand_pose_48[:, :24]  # (N,24)
    left_pca  = hand_pose_48[:, 24:]  # (N,24)

    right_hand = right_pca @ comp_r.T  # (N,45)
    left_hand  = left_pca  @ comp_l.T  # (N,45)

    return torch.from_numpy(np.concatenate([right_hand, left_hand], axis=1)).float()
...
def convert_smplx_to_pos(smplx: Tuple, same_betas: bool=False) -> np.ndarray:
    ...
    pca_hand_pose = torch.from_numpy(pose_seq[:, 69:]).float()
    hand_pose = hand_pca_to_axis_angle(pca_hand_pose)
    # print(hand_pose.shape)
    betas = torch.from_numpy(np.array(betas)).float().unsqueeze(0)
    if same_betas:
        betas = betas * 0.0
